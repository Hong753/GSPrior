...
class GaussianModel:
    ...
    def set_tsdf_test(self, exp_name=None,iteration=None,interp=None):
        self.tsdf = np.load('./out/{}/tsdf_{}_{}.npy'.format(exp_name,exp_name,iteration))
        self.vol_origin = np.load('./out/{}/vol_origin_{}_{}.npy'.format(exp_name,exp_name,iteration))
    ...
    def get_opacity(self, iteration=None,tsdf_mask=False,exp_name=None):
        exp_name = self.exp_name
        if iteration is None or self.gs_state:
            return self.opacity_activation(self._opacity)
        else:
            gs_pts = self.get_xyz.clone().detach().cpu().numpy()
            gs_pts = (gs_pts - self.vol_origin) / self.voxel_size
            Dx, Dy, Dz = self.tsdf.shape
            gs_pts[:,0] = 2.0 * gs_pts[:,0] / (Dx-1) - 1.0
            gs_pts[:,1] = 2.0 * gs_pts[:,1] / (Dy-1) - 1.0
            gs_pts[:,2] = 2.0 * gs_pts[:,2] / (Dz-1) - 1.0
            tsdf = torch.from_numpy(self.tsdf[None, None]).float().to(self._opacity.device)
            gs_pts = torch.from_numpy(gs_pts[None, :, None, :]).float().to(self._opacity.device)
            with torch.no_grad():
                sdf_interp = self.interp(tsdf, gs_pts).reshape(-1,1)
            op_prior =self.opacity_activation(self._opacity)
            if(tsdf_mask):
                return sdf_interp,op_prior
            else:
                return op_prior
    ...
    def reset_opacity(self,iteration):
        opacities_new = inverse_sigmoid(torch.min(self.get_opacity(iteration=iteration), torch.ones_like(self.get_opacity(iteration=iteration))*0.01))
    ...
    def cat_tensors_to_optimizer_my(self):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            assert len(group["params"]) == 1
            stored_state = self.optimizer.state.get(group['params'][0], None)
            if stored_state is not None:
                ...
                optimizable_tensors[group["name"]] = group["params"][0]
        return optimizable_tensors
    def densification_postfix_my(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation):
        d = {"xyz": new_xyz,
        "f_dc": new_features_dc,
        "f_rest": new_features_rest,
        "opacity": new_opacities,
        ...
        optimizable_tensors = self.cat_tensors_to_optimizer_my()
        self._xyz = optimizable_tensors["xyz"]
        self._features_dc = optimizable_tensors["f_dc"]
        self._features_rest = optimizable_tensors["f_rest"]
        self._opacity = optimizable_tensors["opacity"]
    ...
    def add_random_points(self, num_new=8000, N=1):
        n_init_points = self.gt_surf.shape[0]
        device = self.gt_surf.device
        idx = torch.randint(0, n_init_points, (num_new,), device=device)
        stds = self.get_scaling[idx].mean(dim=0, keepdim=True).repeat(num_new, 1)
        means = torch.zeros((stds.size(0), 3), device=device)
        samples = torch.normal(mean=means, std=stds)
        rots = torch.eye(3, device=device).unsqueeze(0).repeat(num_new, 1, 1)
        new_xyz = torch.bmm(rots, samples.unsqueeze(-1)).squeeze(-1) + self.get_xyz[idx].repeat(N, 1)
        new_scaling = self.scaling_inverse_activation(self.get_scaling[idx].repeat(N,1) / (0.8*N))
        new_rotation = self._rotation[idx].repeat(N,1)
        new_features_dc = self._features_dc[idx].repeat(N,1,1)
        new_features_rest = self._features_rest[idx].repeat(N,1,1)
        new_opacity = torch.full((num_new * N, 1), 0.8, device=device)
        new_knn_f = torch.zeros_like(self._knn_f[idx].repeat(N, 1))
        self.densification_postfix(new_xyz, new_knn_f, new_features_dc, new_features_rest,
                                new_opacity, new_scaling, new_rotation)
        print(f"Added {num_new} new points. Total now: {self.get_xyz.shape[0]}")
    ...
    @torch.no_grad()
    def get_sdf(self,gs_pts):
        gs_pts = gs_pts.clone().detach().cpu().numpy()
        gs_pts = (gs_pts - self.vol_origin) / self.voxel_size
        Dx, Dy, Dz = self.tsdf.shape
        gs_pts[:,0] = 2.0 * gs_pts[:,0] / (Dx-1) - 1.0
        gs_pts[:,1] = 2.0 * gs_pts[:,1] / (Dy-1) - 1.0
        ...
        return sdf_interp
    @torch.no_grad()
    def compute_sdf_gradient(self, point, h=1e-3):
        N = point.shape[0]
        grad = torch.zeros(N, 3).to(point.device)
        sdf = self.get_sdf(point)
        for i in range(3):
            delta = torch.zeros_like(point)
            delta[:, i] = h
            sdf_plus = self.get_sdf(point + delta)
            sdf_minus = self.get_sdf(point - delta)
            grad[:, i] = ((sdf_plus - sdf_minus) / (2 * h)).squeeze()
        return grad,sdf
    def densify_and_prune(self, max_grad, abs_max_grad, min_opacity, extent, max_screen_size,iteration):
        if(iteration>30000 and self.gs_state==False):
            if(iteration%1000==0):
                ...
                g_norm = torch.norm(grads, dim=-1)
                active_mask=(grads_abs>0.0001*torch.mean(g_norm)).squeeze()
                unactive_mask = ~active_mask
                tsdf,_ = self.get_opacity(iteration=iteration,tsdf_mask=True)
                tsdf_mask1 = (tsdf == 1.0).squeeze()
                tsdf_mask2 = (tsdf == -1.0).squeeze()
                tsdf_mask  = torch.logical_and(torch.logical_or(tsdf_mask1, tsdf_mask2),unactive_mask)
                num_true = tsdf_mask.sum().item()
                self.prune_points(tsdf_mask)
                grad,sdf = self.compute_sdf_gradient(point=self.get_xyz)
                grad_norm = F.normalize(grad, dim=-1)
                self._xyz = self.get_xyz - 1e-7*(sdf * grad_norm)
            else:
                grads = self.xyz_gradient_accum / self.denom
                grads_abs = self.xyz_gradient_accum_abs / self.denom_abs
                grads[grads.isnan()] = 0.0
