...
from utils.tsdf_prior_gen import *
...
def gen_virtul_cam(cam, trans_noise=1.0, deg_noise=15.0):
    ...
    return virtul_cam
def gs_mode_swither(iteration, tsdf_iter):
    if(iteration>=tsdf_iter[0]):
        return False
    return True
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,exp_name,tsdf_iter,tsdf_trunc,gs_mode=True):
    first_iter = 0
    ...
    tsdf_map = dict(zip(tsdf_iter, tsdf_trunc))
    os.makedirs(debug_path, exist_ok=True)

    for iteration in range(first_iter, opt.iterations + 1):
        gs_mode = gs_mode_swither(iteration, tsdf_iter=tsdf_iter)
        gaussians.gs_state = gs_mode
        ...
        ###   query_points, tsdf_vol, mesh_fusion, mesh_o3d, vol_origin, voxel_size,sampled_pts
        if iteration in tsdf_map:
            tsdf_trunc = tsdf_map[iteration]
            tsdf, vol_origin, voxel_size, mesh = run_tsdf_fusion(
                model_path='./out/{}'.format(exp_name),
                source_path='/data/yth/dataset/DTU-dataset/DTU/scan122',sdf_trunc = tsdf_trunc,
                voxel_size=0.005)
            np.save('tsdf_{}.npy'.format(exp_name),tsdf)
            np.save('vol_origin_{}.npy'.format(exp_name),vol_origin)
            mesh.export("tt_{}.ply".format(exp_name))
        ...
        if iteration > opt.single_view_weight_from_iter:
            ...
            loss += (normal_loss)
        ### TSDF loss
        if gs_mode==False:#iteration > tsdf_iter:
            sdf_interp, op = gaussians.get_opacity(iteration=iteration, tsdf_mask=True)
            band = 0.3
            sdf_interp = torch.where(sdf_interp.abs() <= band,
                                    torch.ones_like(sdf_interp),  # 区间内直接=1
                                    sdf_interp)                   # 区间外保持原值
            # ✅ 使用 softsign 的导数作为 mask，始终非负
            mask_surface = 1.0 / (1.0 + torch.abs(sdf_interp))**2  
            # 表面附近 (mask≈1) 推向 opacity=1
            loss_surface = (((op - 1.0) ** 2) * mask_surface).mean()
            # 表面之外
            mask_free = 1.0 - mask_surface
            loss_free = ((op ** 2) * mask_free).mean()
            loss += 1e-3*loss_surface + 1e-3*loss_free
        ...
        if iteration > opt.multi_view_weight_from_iter:
            ...
            if nearest_cam is not None:
                ...
                nearest_render_pkg = render(nearest_cam, gaussians, pipe, bg, app_model=app_model,
                                            return_plane=True, return_depth_normal=False,iteration=iteration)
        ...
        with torch.no_grad():
            ...
            # Densification for 
            if gs_mode and iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval_gs== 0:
                    size_threshold = 20 if iteration > opt.opacity_reset_interval else None
                    gaussians.densify_and_prune(opt.densify_grad_threshold, opt.densify_abs_grad_threshold, 
                                                opt.opacity_cull_threshold, scene.cameras_extent, size_threshold,iteration=iteration)
                
                # multi-view observe trim
                # if opt.use_multi_view_trim and iteration % 1000 == 0:
                #     observe_the = 2
                #     observe_cnt = torch.zeros_like(gaussians.get_opacity(iteration=iteration))
                #     for view in scene.getTrainCameras():
                #         render_pkg_tmp = render(view, gaussians, pipe, bg, app_model=app_model, return_plane=False, return_depth_normal=False,iteration=iteration)
                ...
                if iteration < opt.densify_until_iter:
                    if iteration % opt.opacity_reset_interval == 0 or (dataset.white_background and iteration == opt.densify_from_iter):
                        gaussians.reset_opacity(iteration)
            else:
                #     Keep track of max radii in image-space for pruning
                mask = (render_pkg["out_observe"] > 0) & visibility_filter
                gaussians.max_radii2D[mask] = torch.max(gaussians.max_radii2D[mask], radii[mask])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[1000,5000,7000,9000,15000,20000,25000,30000,31000, 32000,
                                                                           33000, 34000, 35000, 36000, 37000, 38000,
                                                                           39000, 40000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[1000,5000,7000,9000,15000,20000,25000,30000,31000, 32000,
                                                                           33000, 34000, 35000, 36000, 37000, 38000,
                                                                           39000, 40000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[20000,30000,40000])
    parser.add_argument("--start_checkpoint", type=str, default = '')
    parser.add_argument('--tsdf_iter', type=int, default=[20000,30000,40000])
    parser.add_argument('--tsdf_trunc', type=int, default=[24*4,12*4,6*4])
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations
             , args.start_checkpoint, args.debug_from,exp_name, args.tsdf_iter, args.tsdf_trunc)
